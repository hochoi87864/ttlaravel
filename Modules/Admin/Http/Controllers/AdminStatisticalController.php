...
class AdminStatisticalController extends Controller
{
    ...
    public function getStatistical(Request $request){
        $statistical_date_start = date("d-m-Y H:i:s", strtotime($request->statistical_date_start));
        $statistical_date_end = date("d-m-Y H:i:s", strtotime($request->statistical_date_end));
